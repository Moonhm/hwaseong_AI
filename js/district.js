 ...
 *   (2026-09-02: guFromAddress 에 DONG_TO_GU 폴백을 넣어 주차장 '구' 인식만
 *    72/131 → 131/131 이 됐다. 읍면동 단위로 나누자는 결론이 바뀐 것은 아니다.)
 ...
 *
 * 동 단위는 2026-09-02 에 같은 방법으로 채웠다 — js/restaurants-static.json 에서
 * '구' 와 괄호 안 법정동이 함께 있는 주소를 세었고(동탄·병점 주소는 동이 문장 끝
 * 괄호에 들어 있다) 전부 한 구로만 나왔다. 표본 없이 넣은 것은 넷뿐이다:
 *   능동·오산동·목동·중동  — 주차장 주소가 '화성시 동탄구 … (능동 1113-4)' 처럼
 *   구와 함께 적어 두어 그것으로 확인했다.
 * ⚠ 반월동은 동탄 생활권이지만 **병점구**다(음식점 109건 전건). 지도만 보고 고치지 마라.
 * ⚠ '신동' 은 넣지 않는다 — '신동탄' 주소 24건에 오탐한다. '상가동' 도 넣지 않는다(건물 표기).
 */
var DONG_TO_GU = {
  '경기동': '동탄구', '동탄치동': '동탄구',
  '반송동': '동탄구', '석우동': '동탄구', '능동': '동탄구', '오산동': '동탄구',
  '영천동': '동탄구', '산척동': '동탄구', '방교동': '동탄구', '장지동': '동탄구',
  '목동': '동탄구', '중동': '동탄구', '여울동': '동탄구', '청계동': '동탄구',
  '송동': '동탄구', '금곡동': '동탄구',
  '병점동': '병점구', '안녕동': '병점구', '진안동': '병점구', '기산동': '병점구',
  '반월동': '병점구', '반정동': '병점구', '송산동': '병점구',
  '기안동': '효행구', '배양동': '효행구',
  '매송면': '효행구', '봉담읍': '효행구', '비봉면': '효행구', '정남면': '효행구',
  '새솔동': '만세구',
  '남양읍': '만세구', '마도면': '만세구', '서신면': '만세구', '송산면': '만세구',
  '양감면': '만세구', '우정읍': '만세구', '장안면': '만세구', '팔탄면': '만세구', '향남읍': '만세구',
};

/* ⚠ \b 를 쓰면 안 된다. \b 는 \w=[A-Za-z0-9_] 기준 경계라 한글 뒤에서는
 * 양쪽 모두 non-word 라 경계가 성립하지 않는다 — 즉 '동탄구' 다음이 공백이든
 * 문자열 끝이든 절대 매치되지 않는다. 두 그룹이 선택적이라 정규식 자체는
 * '화성시 ' 에서 성공하고 캡처만 undefined 가 되어, guFromAddress 가 조용히
 * 100% null 을 돌려줬다(2026-08-26 감사에서 3,754/3,754 전건 실패 확인).
 * 한글 부정 전방탐색으로 바꾼다. */
var _GU_RE = /화성시\s+(?:([가-힣]+구)(?![가-힣]))?\s*(?:([가-힣]{2,6}(?:읍|면|동))(?![가-힣]))?/;

/* 폴백용 키. 긴 것부터 본다 — '반송동' 이 '송동' 보다, '매송면' 이 '목동'('야목동길')
 * 보다 먼저 걸려야 한다. 짧은 키가 먼저 걸리면 엉뚱한 구가 나온다. */
var _DONG_KEYS = Object.keys(DONG_TO_GU).sort(function (a, b) { return b.length - a.length; });

/* 주소 문자열에서 구를 뽑는다. 못 뽑으면 null. */
function guFromAddress(addr) {
  var s = String(addr || '').replace('경기 ', '경기도 ');
  var m = _GU_RE.exec(s);
  if (m) {
    if (m[1] && GU_NAMES.indexOf(m[1]) >= 0) return m[1];
    if (m[2] && DONG_TO_GU[m[2]]) return DONG_TO_GU[m[2]];
  }
  /* ⚠ 정규식이 못 잡은 것만 여기로 온다. 먼저 걸면 '동탄구 반송동' 같은 정상 주소에서
   * 순서가 뒤집힌다. 주차장 주소가 셋 다 이 꼴이라 131건 중 59건이 여기로 온다:
   *   읍면과 리가 붙음    '팔탄면가재리1-19'            → (?![가-힣]) 부정 전방탐색에 막힌다
   *   괄호 안 읍면동      '시청로 895-20 (팔탄면,율암리)' → '화성시' 뒤가 도로명이다
   *   '화성시' 자체가 없음 '동탄구 석우동 648'            → exec 가 null (그래서 위 if 밖이다)
   * 못 잡으면 그대로 null 을 돌려주고 guOf 가 guFromBox 로 떨어진다 — 순서는 그대로다. */
  for (var i = 0; i < _DONG_KEYS.length; i++) {
    if (s.indexOf(_DONG_KEYS[i]) >= 0) return DONG_TO_GU[_DONG_KEYS[i]];
  }
  return null;
}
