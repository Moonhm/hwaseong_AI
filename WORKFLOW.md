...
| **지도가 안 뜬다 — 카카오 웹 도메인 미등록** | 2026-08-31 터널을 새로 띄우면서 새 주소가 **Kakao Developers 에 등록되지 않았다.** SDK 가 `401 domain mismatched` 로 거부돼 지도 탭이 통째로 죽는다 — 화면은 *「페이지를 새로고침 해주세요」* 라고 **원인을 잘못 안내한다.** **Claude 가 못 고친다: 콘솔 로그인이 필요하다.** 사용자가 앱 설정 → 플랫폼 → Web → 사이트 도메인에 새 주소(+ `http://localhost:8080`)를 등록하면 즉시 해소된다. 절차·실측은 §12 「Kakao API」 · [`docs/log/2026-08-31-deploy-kakao-domain.md`](docs/log/2026-08-31-deploy-kakao-domain.md) |
...
> 해결된 항목은 §14 규칙대로 **지웁니다.** 2026-08-31 에 「배포 URL 죽음」을 지웠습니다 —
