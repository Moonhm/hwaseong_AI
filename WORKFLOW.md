...
- **amend 를 했으면 해시를 다시 읽어 고치십시오.** `git log -1 --format=%h` 로
  확인하고, 그 수정은 **뒤따르는 커밋**에 넣습니다. 고치겠다고 또 amend 하면 또 어긋납니다.
- **amend 할 생각이면 해시를 나중에 적으십시오.** 모양이 확정된 뒤 한 번만 적습니다.
