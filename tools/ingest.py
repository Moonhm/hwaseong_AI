...
VALUE_SETS = {
    ...
    # 관광진흥법상 관광편의시설업 — 업종 값이 그대로 들어온다
    "touristfacility": {"관광극장식당업", "관광펜션업", "외국인전용유흥음식점업", "관광유흥음식점업",
                        "관광식당업", "관광순환버스업", "관광사진업", "여객자동차터미널시설업",
                        "한옥체험업", "관광궤도업", "관광면세업", "관광지원서비스업"},
    "travel":      {"국내여행업", "국외여행업", "일반여행업", "종합여행업", "국내외여행업"},
    "cinema":      {"영화상영관"},
    "heritage":    {"국가지정문화재", "도지정문화재", "시지정문화재", "국가등록문화재",
                    "향토문화재", "문화재자료", "등록문화재"},
}
COL_HINTS = [
    # ⚠ 순서가 곧 우선순위다. 좁은 단서를 위에 둔다.
    #   '업종' 처럼 여러 데이터에 공통으로 나오는 컬럼명은 단독 단서로 쓰면 안 된다 —
    #   실제로 관광편의시설업·여행업·영화상영관 3종이 전부 지역화폐로 오분류됐다.
    ("heritage",    ("국가유산명", "문화재명", "지정번호", "문화재 지정일", "국가유산")),
    ("cinema",      ("상영관명", "상영관소재지", "영화상영관")),
    ("parking",     ("주차구획", "주차면", "급지", "주차장명", "요금정보", "주차장구분",
                     "구간이름", "장애면수", "정기면수", "재입장시간")),
    ("localcurrency", ("가맹점명", "카드가맹", "지역화폐")),
    ("camping",     ("야영장명", "야영장구분", "야영장면수", "캠핑장명", "글램핑", "카라반")),
    ("hotel",       ("객실수", "관광호텔", "호텔명", "부대시설")),
    ("touristrest", ("관광식당", "관광진흥법", "음식종류", "영업장면적")),
    ("restaurant",  ("모범음식점", "주메뉴", "음식점명")),
    ("temple",      ("템플스테이", "사찰명", "수련프로그램", "전통사찰")),
    ("festival",    ("축제명", "행사명", "행사기간", "개최기간", "축제기간")),
    ("tourist",     ("관광지명", "명소", "볼거리", "관광지")),
]
LABEL = {
    ...
    "heritage": "지정문화재", "cinema": "영화상영관",
    "touristfacility": "관광편의시설업", "travel": "여행업",
    "stats": "통계·집계", "_review": "신규 유형 후보(보류)", "_reject": "반입 불가",
}
...
NAME_COLS = ["명칭", "시설명", "장소명", "상호명", "이름", "축제명", "행사명",
             ...
             "관광지명", "체험장명", "시설명칭", "국가유산명", "문화재명",
             "상영관명", "상호", "구간이름", "name"]
ADDR_COLS = ["소재지도로명주소", "소재지지번주소", "도로명주소", "지번주소",
             "주소", "소재지", "위치", "위 치", "장소주소",
             "소재지 도로명주소", "소재지 지번주소", "상영관소재지(도로명)", "address"]
...
def classify(rows):
    ...
    # 4) 통계·집계 — 개별 장소가 아니라 숫자 표
    #    이름 컬럼도 주소도 없는데 나머지가 숫자 위주면 집계 데이터다.
    nc = pick(cols, NAME_COLS, _is_name_col)
    if not ac and not pick(cols, LAT_COLS):
        numish = 0
        for c in cols:
            vals = [str(r.get(c) or "").replace(",", "").strip() for r in rows[:40]]
            vals = [v for v in vals if v]
            if vals and sum(1 for v in vals if v.replace(".", "", 1).isdigit()) / len(vals) >= 0.8:
                numish += 1
        if numish >= max(2, len(cols) // 2):
            return "stats", "이름·주소 없이 숫자 컬럼 %d/%d — 집계표" % (numish, len(cols))

    # 5) 최소 요건조차 없으면 거절
    if not nc and not ac and not pick(cols, LAT_COLS):
